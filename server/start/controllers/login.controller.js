  ...
  try {
    ...
    if (employerLogin) {
      const matchedPassword = bcrypt.compareSync(
        password,
        employerLogin.password,
      );
      if (matchedPassword) {
        const token = jwt.sign(employerLogin, process.env.JWT_SECRET, {
          expiresIn: "20h",
        });
        res.cookie("acess_token", token);
        return res.json({
          success: true,
          message: "Login successful as employer",
        });
      } else {
    ...
    if (seekerLogin) {
      ...
      if (passwordMatch) {
        return res.json({
          success: true,
          message: "Login successful as job seeker",
        });
      } else {
        return res.json({ success: false, message: "Wrong login credentials" });
      }
